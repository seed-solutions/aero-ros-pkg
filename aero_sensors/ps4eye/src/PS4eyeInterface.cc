#include "aero_sensors/PS4eyeInterface.hh"

using namespace ps4eye;
using namespace interface;

//////////////////////////////////////////////////
PS4eyeInterface::PS4eyeInterface(ros::NodeHandle _nh)
    ...
    depth_width_(640), depth_height_(480)
{
  depth_ops_ =
    ros::SubscribeOptions::create<sensor_msgs::PointCloud2>(
        "/stereo/points2",
        1,
        boost::bind(&PS4eyeInterface::DepthCallback, this, _1),
        ros::VoidPtr(),
        &depth_queue_);
  ...
  image_ops_ =
    ros::SubscribeOptions::create<sensor_msgs::Image>(
        ...
        1,
        boost::bind(&PS4eyeInterface::ImageCallback, this, _1),
        ros::VoidPtr(),
        &image_queue_);
...
//////////////////////////////////////////////////
PS4eyeInterface::~PS4eyeInterface()
{
}

//////////////////////////////////////////////////
sensor_msgs::PointCloud2 PS4eyeInterface::ReadPoints()
{
  depth_mutex_.lock();
  auto res = depth_;
  depth_mutex_.unlock();
...
//////////////////////////////////////////////////
sensor_msgs::PointCloud2 PS4eyeInterface::ReadPoints(float _scale_x, float _scale_y)
{
  depth_mutex_.lock();
...
//////////////////////////////////////////////////
sensor_msgs::Image PS4eyeInterface::ReadImage()
{
  image_mutex_.lock();
  auto res = image_;
  image_mutex_.unlock();
...
//////////////////////////////////////////////////
sensor_msgs::PointCloud2 PS4eyeInterface::ReadPointsAfter(float _scale_x, float _scale_y)
{
  ros::Time time;
  for (int i=0; i < 100; ++i) {
    depth_mutex_.lock();
...
//////////////////////////////////////////////////
sensor_msgs::Image PS4eyeInterface::ReadImageAfter()
{
  ros::Time time;
  for (int i=0; i < 100; ++i) {
    image_mutex_.lock();
...
//////////////////////////////////////////////////
std::vector<sensor_msgs::RegionOfInterest> PS4eyeInterface::ImageBounds
...
//////////////////////////////////////////////////
std::vector<geometry_msgs::Point> PS4eyeInterface::ImageCenters
...
//////////////////////////////////////////////////
void PS4eyeInterface::SetNow()
{
  time_now_ = ros::Time::now();
}

//////////////////////////////////////////////////
void PS4eyeInterface::DepthCallback(const sensor_msgs::PointCloud2::ConstPtr& _msg)
{
  depth_mutex_.lock();
  depth_ = *_msg;
  depth_mutex_.unlock();
...
//////////////////////////////////////////////////
void PS4eyeInterface::ImageCallback(const sensor_msgs::Image::ConstPtr& _msg)
{
  image_mutex_.lock();
  image_ = *_msg;
  image_mutex_.unlock();

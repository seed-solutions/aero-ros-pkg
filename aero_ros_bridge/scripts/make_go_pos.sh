...
create_go_pos_from_calib() {
    ...
    for e in "${name[@]}"
    do
	lisp="${lisp}${tab2}(:$e (&optional (x 0) (y 0) (theta 0) (vel 308) (robot *aero*))\n"
	...
	lisp="${lisp}${tab2}${tab2}${tab2}(setq time-theta (/ (*  ${vel[$idx]} ${time[$idx]} theta) (* vel ${rot[$idx]})))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}(if (not (= time-xy 0))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}${tab2}(progn (send self :wheel-vector (scale (/ vel (+ x y)) (float-vector (- x y) (- (+ x y)) (+ x y) (- y x))))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}${tab2}${tab7}(unix::usleep (round (* time-xy 1000)))))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}(if (not (= time-theta 0))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}${tab2}(progn (send self :wheel-vector (float-vector vel (- vel) (- vel) vel))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}${tab2}${tab7}(unix::usleep (round (* time-theta 1000)))))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}(send self :wheel-vector #f(0 0 0 0))\n"
	lisp="${lisp}${tab2}${tab2}${tab2}(list x y theta)))\n"
	idx=$(($idx + 1))
	names[${#names[@]}]="$e"
    done

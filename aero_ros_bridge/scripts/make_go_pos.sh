...
create_go_pos_from_calib() {
    ...
    for e in "${name[@]}"
    do
	lisp="${lisp}${tab2}(:$e (&optional (x 0) (y 0) (theta 0) (exec-large nil) (vel 308.0) (robot *aero*))\n"
...
body="${body}${tab2}(:go-pos (x y theta &optional (exec-large nil) (vel *wheel-velocity*) (robot *aero*))\n"
...
do
    ...
    body="${body}${tab2}${tab2}${tab2}${tab3}(setq res (send self :$e x y theta exec-large (float vel) robot)))\n"
done
...
body="${body}${tab2}${tab2}${tab2}${tab3}(setq res (send self :gp-nil x y theta exec-large (float vel) robot)))\n"

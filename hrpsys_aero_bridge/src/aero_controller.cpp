...
void AeroController::raw_to_stroke_(std::vector<uint8_t>& raw,
...
void AeroController::set_command_header(
    uint8_t id, uint8_t cmd, uint16_t time,
...
/// @param data buffer, MUST be RAW_DATA_LENGTH(77) bytes.
void AeroController::set_check_sum(std::vector<uint8_t>& dat) {
...
void AeroController::servo_command(int16_t d0) {
  ...
  set_command_header(ID_UPPER, CMD_MOTOR_SRV, 0, dat);
...
void AeroController::set_position(std::vector<int16_t>& stroke_vector,
  ...
  std::vector<uint8_t> dat;
  dat.resize(RAW_DATA_LENGTH);
  set_command_header(ID_UPPER, CMD_MOVE_ABS, time, dat);
  stroke_to_raw_(stroke_vector, dat);
  set_check_sum(dat);
  seed_485_send(dat);
}
